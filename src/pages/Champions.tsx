...
import { getActiveCardsForChampions } from '../services/firestore';
...
function MatchCard({
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const predictionDocRef = doc(db, `users/${currentUser!.uid}/tournaments/champions_league_2025/predictions`, card.cardId);
  ...
  const handleModify = async () => {
    ...
    setSaving(true);
    try {
      await updateDoc(doc(db, `users/${currentUser!.uid}/tournaments/champions_league_2025/predictions`, userPrediction.matchId), {
        homeScore: Number(homeScore),
        awayScore: Number(awayScore),
        lockedAt: serverTimestamp()
      });
...
export default function Champions() {
  ...
  useEffect(() => {
    ...
    const q = collection(db, `users/${currentUser.uid}/tournaments/champions_league_2025/predictions`);
  ...
  if (loading) {
    return (
      <div className="fade-in">
        <h1 className="page-title">🏆 Champions League</h1>
  ...
  return (
    <div className="fade-in">
      ...
      <p style={{ color: 'var(--text-muted)', marginBottom: '2rem', fontSize: '0.95rem' }}>
        Predice el marcador exacto de los partidos de Champions League. Cada predicción cuesta tokens. Tienes 48 horas para modificar tu marcador antes de que se bloquee definitivamente.
      </p>
