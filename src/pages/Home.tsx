...
export default function Home() {
    ...
    const submitPrediction = async (e: React.FormEvent) => {
        ...
        const isConfirmed = window.confirm(
            "¿Estás seguro de tu marcador exacto?\n\n" + 
            "Una vez enviada la predicción, no podrás modificarla desde tu panel. Para cualquier cambio o error tipográfico deberás contactar directamente al Administrador."
        );
    ...
    useEffect(() => {
        const unsub = onSnapshot(doc(db, "system", "radar_match"), (docSnap) => {
            if (docSnap.exists()) {
                const data = docSnap.data();
                setMatchData({
                    teams: {
                        home: { name: data.homeTeam, logo: data.homeFlag },
                        away: { name: data.awayTeam, logo: data.awayFlag }
                    },
            ...
            console.error("Error listening to radar_match:", error);
        });
        ...
        <div className="fade-in">
            ...
            <div style={{ marginTop: '2rem' }}>
                <h2 style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '1.5rem' }}>
                    <Activity color="#FFD700" /> Radar Tricolor en Vivo
                </h2>
                
                {matchData ? (
                    <div>
                        <div className="glass-card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', textAlign: 'center' }}>
                            <div style={{ flex: 1 }}>
                        ...
                        {prediction && (
                            <div style={{ display: 'flex', gap: '10px', marginTop: '1rem' }}>
                                <div style={{ flex: 1, background: 'rgba(0, 255, 136, 0.1)', padding: '10px', borderRadius: '12px', textAlign: 'center', border: '1px solid rgba(0,255,136,0.3)' }}>
                                    ...
                                    <div style={{ fontWeight: 800, color: '#00FF88' }}>{prediction.predictions.percent.home}</div>
                                </div>
                                <div style={{ flex: 1, background: 'rgba(255, 215, 0, 0.1)', padding: '10px', borderRadius: '12px', textAlign: 'center', border: '1px solid rgba(255,215,0,0.3)' }}>
                                    ...
                                    <div style={{ fontWeight: 800, color: 'var(--primary)' }}>{prediction.predictions.percent.draw}</div>
                                </div>
                                <div style={{ flex: 1, background: 'rgba(255, 0, 85, 0.1)', padding: '10px', borderRadius: '12px', textAlign: 'center', border: '1px solid rgba(255,0,85,0.3)' }}>
                                    ...
                                    <div style={{ fontWeight: 800, color: 'var(--accent-rd)' }}>{prediction.predictions.percent.away}</div>
                                </div>
                            </div>
                        )}

                        {/* ZONA DE APUESTA */}
                        <div className="glass-card" style={{ marginTop: '2rem', border: '1px solid rgba(255, 215, 0, 0.3)' }}>
                            ...
                            <form onSubmit={submitPrediction}>
                                ...
                                <button type="submit" className="button-primary" style={{ width: '100%' }} disabled={isSubmitting}>
                                    {isSubmitting ? 'Registrando en la Blockchain...' : 'Enviar Marcador Cerrado'}
                                </button>
                            </form>
                        </div>
                    </div>
            ...
            <div className="glass-card" style={{ marginTop: '2rem' }}>
                ...
                <p style={{ marginTop: '10px', color: 'var(--text-muted)' }}>
                    Recuerda que todas las predicciones enviadas están sujetas al pago de la inscripción. El administrador actualizará tu estado de "Pendiente" a "Pagado" una vez confirmado el abono.
                </p>
            </div>
        </div>
    );
